...
namespace Terraria.World.Generation
{
    public static class Modifiers
    {
                ...
                if (GenBase._random.NextDouble() > (double) Math.Max(0.0f,
                        Math.Min(1f,
                            (float) (((double) Vector2.Distance(new Vector2((float) x, (float) y), vector2) -
                                      (double) this._innerRadius) /
                                     ((double) this._outerRadius - (double) this._innerRadius)))))
            ...
            public override bool Apply(Point origin, int x, int y, params object[] args)
            {
                ...
                while (index < num)
                {
                    if (!GenBase._tiles[x + Modifiers.IsTouchingAir.DIRECTIONS[index],
                        y + Modifiers.IsTouchingAir.DIRECTIONS[index + 1]].active())
                        return this.UnitApply(origin, x, y, args);
                    index += 2;
                }
        ...
        public class RectangleMask : GenAction
        {
            ...
            public override bool Apply(Point origin, int x, int y, params object[] args)
            {
                if (x >= this._xMin + origin.X && x <= this._xMax + origin.X &&
                    (y >= this._yMin + origin.Y && y <= this._yMax + origin.Y))
                    return this.UnitApply(origin, x, y, args);
                return this.Fail();
            }
        }
