...
namespace Terraria
{
    public class DeprecatedClassLeftInForLoading
    {
        ...
        public static void UpdateDummies()
        {
            ...
            for (var index1 = 0; index1 < 1000; ++index1)
            {
                if (DeprecatedClassLeftInForLoading.dummies[index1] != null)
                {
                    ...
                    {
                        if (!Main.npc[DeprecatedClassLeftInForLoading.dummies[index1].npc].active ||
                            Main.npc[DeprecatedClassLeftInForLoading.dummies[index1].npc].type != 488 ||
                            ((double) Main.npc[DeprecatedClassLeftInForLoading.dummies[index1].npc].ai[0] !=
                             (double) DeprecatedClassLeftInForLoading.dummies[index1].x ||
                             (double) Main.npc[DeprecatedClassLeftInForLoading.dummies[index1].npc].ai[1] !=
                             (double) DeprecatedClassLeftInForLoading.dummies[index1].y))
        ...
        public static int Find(int x, int y)
        {
            for (var index = 0; index < 1000; ++index)
            {
                if (DeprecatedClassLeftInForLoading.dummies[index] != null &&
                    ...
                    (int) DeprecatedClassLeftInForLoading.dummies[index].y == y)
                    return index;
            }
        ...
        public void Activate()
        {
            ...
            if (Main.netMode == 1)
                return;
            NetMessage.SendData(86, -1, -1, (NetworkText) null, this.whoAmI, (float) this.x, (float) this.y, 0.0f, 0, 0,
                0);
        }
