    ...
    public class Rain
    {
        ...
        public static void MakeRain()
        {
            ...
            for (int index = 0; (double) index < (double) num1; ++index)
            {
                ...
                Vector2 Position;
                Position.X = (float) Main.rand.Next((int) Main.screenPosition.X - num2,
                    (int) Main.screenPosition.X + Main.screenWidth + num2);
        ...
        public void Update()
        {
            ...
            if (!Collision.SolidCollision(this.position, 2, 2) &&
                (double) this.position.Y <= (double) Main.screenPosition.Y + (double) Main.screenHeight + 100.0 &&
                !Collision.WetCollision(this.position, 2, 2))
                return;
            this.active = false;
