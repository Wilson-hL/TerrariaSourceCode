...
namespace Terraria.Graphics.Shaders
{
    public class ScreenShaderData : ShaderData
    {
        ...
        {
            get { return this._uIntensity; }
        }

        public float CombinedOpacity
        {
            get { return this._uOpacity * this._globalOpacity; }
        }
            ...
            for (int index = 0; index < this._uImages.Length; ++index)
            {
                if (this._uImages[index] != null && this._uImages[index].Value != null)
                {
                    ...
                    Main.graphics.GraphicsDevice.SamplerStates[index + 1] = this._samplerStates[index] == null
                        ...
                        : this._samplerStates[index];
                    this.Shader.Parameters["uImageSize" + (object) (index + 1)]
                        .SetValue(new Vector2((float) width, (float) height) * this._imageScales[index]);
                }
            }
