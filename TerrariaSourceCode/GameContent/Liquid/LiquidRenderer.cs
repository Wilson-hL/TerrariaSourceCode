...
namespace Terraria.GameContent.Liquid
{
    public class LiquidRenderer
    {
        ...
        private static readonly byte[] WAVE_MASK_STRENGTH = new byte[5]
            {(byte) 0, (byte) 0, (byte) 0, byte.MaxValue, (byte) 0};

        private static readonly byte[] VISCOSITY_MASK = new byte[5]
            {(byte) 0, (byte) 200, (byte) 240, (byte) 0, (byte) 0};
        ...
        {
            for (var index = 0; index < this._liquidTextures.Length; ++index)
        ...
        private unsafe void InternalPrepareDraw(Rectangle drawArea)
        {
            ...
            fixed (LiquidRenderer.LiquidCache* liquidCachePtr1 = &this._cache[1])
            {
                ...
                var liquidCachePtr3 = liquidCachePtr1 + num1;
                ...
                for (var index1 = 2; index1 < rectangle.Width - 2; ++index1)
                {
                    for (var index2 = 2; index2 < rectangle.Height - 2; ++index2)
                    {
                        var val1 = 0.0f;
                        float num2;
                        ...
                        else if (!liquidCachePtr3->HasLiquid)
                        {
                            ...
                            if (liquidCache1.HasLiquid && liquidCache2.HasLiquid &&
                                (int) liquidCache1.Type == (int) liquidCache2.Type)
                            {
                            ...
                            if (liquidCache3.HasLiquid && liquidCache4.HasLiquid &&
                                (int) liquidCache3.Type == (int) liquidCache4.Type)
                            {
                ...
                var liquidCachePtr4 = liquidCachePtr1;
                for (var index1 = 0; index1 < rectangle.Width; ++index1)
                {
                    for (var index2 = 0; index2 < rectangle.Height - 10; ++index2)
                    {
                        if (liquidCachePtr4->HasVisibleLiquid && !liquidCachePtr4->IsSolid)
                        {
                            ...
                            var num2 =
                                1f / (float) (LiquidRenderer.WATERFALL_LENGTH[(int) liquidCachePtr4->Type] + 1);
                            var num3 = 1f;
                            for (var index3 = 1;
                                index3 <= LiquidRenderer.WATERFALL_LENGTH[(int) liquidCachePtr4->Type];
                                ++index3)
                            {
                                num3 -= num2;
                                if (!liquidCachePtr4[index3].IsSolid)
                                {
                                    liquidCachePtr4[index3].VisibleLiquidLevel = Math.Max(
                                        liquidCachePtr4[index3].VisibleLiquidLevel,
                                        liquidCachePtr4->VisibleLiquidLevel * num3);
                ...
                var liquidCachePtr5 = liquidCachePtr1 + num1;
                for (var index1 = 2; index1 < rectangle.Width - 2; ++index1)
                {
                    for (var index2 = 2; index2 < rectangle.Height - 2; ++index2)
                    {
                        ...
                        else
                        {
                            ...
                            var num4 = 0.0f;
                            var num5 = 1f;
                            var visibleLiquidLevel = liquidCachePtr5->VisibleLiquidLevel;
                            ...
                            var zero = Point.Zero;
                            liquidCachePtr5->HasTopEdge =
                                !liquidCache1.HasVisibleLiquid && !liquidCache1.IsSolid || (double) num4 != 0.0;
                            liquidCachePtr5->HasBottomEdge =
                                !liquidCache2.HasVisibleLiquid && !liquidCache2.IsSolid || (double) num5 != 1.0;
                            liquidCachePtr5->HasLeftEdge =
                                !liquidCache3.HasVisibleLiquid && !liquidCache3.IsSolid || (double) num2 != 0.0;
                            liquidCachePtr5->HasRightEdge =
                                !liquidCache4.HasVisibleLiquid && !liquidCache4.IsSolid || (double) num3 != 1.0;
                            ...
                            if (zero.Y == 16 && liquidCachePtr5->HasLeftEdge ^ liquidCachePtr5->HasRightEdge &&
                                (index2 + rectangle.Y) % 2 == 0)
                                zero.Y += 16;
                ...
                var liquidCachePtr6 = liquidCachePtr1 + num1;
                for (var index1 = 2; index1 < rectangle.Width - 2; ++index1)
                {
                    for (var index2 = 2; index2 < rectangle.Height - 2; ++index2)
                    {
                        if (liquidCachePtr6->HasVisibleLiquid)
                        {
                            ...
                            if (liquidCache1.HasVisibleLiquid && liquidCache2.HasVisibleLiquid)
                            {
                                if (liquidCachePtr6->HasLeftEdge)
                                    liquidCachePtr6->VisibleLeftWall =
                                        (float) (((double) liquidCachePtr6->LeftWall * 2.0 +
                                                  (double) liquidCache1.LeftWall + (double) liquidCache2.LeftWall) *
                                                 0.25);
                                if (liquidCachePtr6->HasRightEdge)
                                    liquidCachePtr6->VisibleRightWall =
                                        (float) (((double) liquidCachePtr6->RightWall * 2.0 +
                                                  (double) liquidCache1.RightWall + (double) liquidCache2.RightWall) *
                                                 0.25);
                            }
                            ...
                            if (liquidCache3.HasVisibleLiquid && liquidCache4.HasVisibleLiquid)
                            {
                                if (liquidCachePtr6->HasTopEdge)
                                    liquidCachePtr6->VisibleTopWall =
                                        (float) (((double) liquidCachePtr6->TopWall * 2.0 +
                                                  (double) liquidCache3.TopWall + (double) liquidCache4.TopWall) *
                                                 0.25);
                                if (liquidCachePtr6->HasBottomEdge)
                                    liquidCachePtr6->VisibleBottomWall =
                                        (float) (((double) liquidCachePtr6->BottomWall * 2.0 +
                                                  (double) liquidCache3.BottomWall + (double) liquidCache4.BottomWall) *
                                                 0.25);
                            }
                        }
                ...
                var liquidCachePtr7 = liquidCachePtr1 + num1;
                for (var index1 = 2; index1 < rectangle.Width - 2; ++index1)
                {
                    for (var index2 = 2; index2 < rectangle.Height - 2; ++index2)
                    {
                        if (liquidCachePtr7->HasLiquid)
                        {
                            ...
                            if (liquidCachePtr7->HasTopEdge && !liquidCachePtr7->HasBottomEdge &&
                                liquidCachePtr7->HasLeftEdge ^ liquidCachePtr7->HasRightEdge)
                            {
                ...
                var liquidCachePtr8 = liquidCachePtr1 + num1;
                for (var index1 = 2; index1 < rectangle.Width - 2; ++index1)
                {
                    for (var index2 = 2; index2 < rectangle.Height - 2; ++index2)
                    {
                        if (liquidCachePtr8->HasLiquid)
                        {
                            ...
                            if (!liquidCachePtr8->HasBottomEdge && !liquidCachePtr8->HasLeftEdge &&
                                (!liquidCachePtr8->HasTopEdge && !liquidCachePtr8->HasRightEdge))
                            {
                                if (liquidCache3.HasTopEdge && liquidCache1.HasLeftEdge)
                                {
                                    liquidCachePtr8->FrameOffset.X =
                                        Math.Max(4, (int) (16.0 - (double) liquidCache1.VisibleLeftWall * 16.0)) - 4;
                                    liquidCachePtr8->FrameOffset.Y =
                                        48 + Math.Max(4,
                                            (int) (16.0 - (double) liquidCache3.VisibleTopWall * 16.0)) - 4;
                                ...
                                {
                                    liquidCachePtr8->FrameOffset.X =
                                        32 - Math.Min(16, (int) ((double) liquidCache1.VisibleRightWall * 16.0) - 4);
                                    liquidCachePtr8->FrameOffset.Y =
                                        48 + Math.Max(4,
                                            (int) (16.0 - (double) liquidCache4.VisibleTopWall * 16.0)) - 4;
                                    liquidCachePtr8->VisibleLeftWall = 0.0f;
                                    liquidCachePtr8->VisibleTopWall = 0.0f;
                                    liquidCachePtr8->VisibleRightWall = 1f;
                                    liquidCachePtr8->VisibleBottomWall = 1f;
                ...
                fixed (Color* colorPtr1 = &this._waveMask[0])
                {
                    ...
                    for (var index1 = 2; index1 < rectangle.Width - 2; ++index1)
                    {
                        for (var index2 = 2; index2 < rectangle.Height - 2; ++index2)
                        {
                            ...
                            {
                                var num2 = Math.Min(0.75f, liquidCachePtr9->VisibleLeftWall);
                                var num3 = Math.Max(0.25f, liquidCachePtr9->VisibleRightWall);
                                var num4 = Math.Min(0.75f, liquidCachePtr9->VisibleTopWall);
                                var num5 = Math.Max(0.25f, liquidCachePtr9->VisibleBottomWall);
                                if (liquidCachePtr9->IsHalfBrick && (double) num5 > 0.5)
                                    num5 = 0.5f;
                                liquidDrawCachePtr2->IsVisible =
                                    liquidCachePtr9->HasWall ||
                                    (!liquidCachePtr9->IsHalfBrick || !liquidCachePtr9->HasLiquid);
                                liquidDrawCachePtr2->SourceRectangle = new Rectangle(
                                    (int) (16.0 - (double) num3 * 16.0) + liquidCachePtr9->FrameOffset.X,
                                    (int) (16.0 - (double) num5 * 16.0) + liquidCachePtr9->FrameOffset.Y,
                                    (int) Math.Ceiling(((double) num3 - (double) num2) * 16.0),
                                    (int) Math.Ceiling(((double) num5 - (double) num4) * 16.0));
                                liquidDrawCachePtr2->IsSurfaceLiquid =
                                    liquidCachePtr9->FrameOffset.X == 16 && liquidCachePtr9->FrameOffset.Y == 0 &&
                                    (double) (index2 + rectangle.Y) > Main.worldSurface - 40.0;
                                liquidDrawCachePtr2->Opacity = liquidCachePtr9->Opacity;
                                liquidDrawCachePtr2->LiquidOffset = new Vector2(
                                    (float) Math.Floor((double) num2 * 16.0), (float) Math.Floor((double) num4 * 16.0));
                                liquidDrawCachePtr2->Type = liquidCachePtr9->VisibleType;
                                liquidDrawCachePtr2->HasWall = liquidCachePtr9->HasWall;
                                var num6 = LiquidRenderer.WAVE_MASK_STRENGTH[(int) liquidCachePtr9->VisibleType];
                                var num7 = (byte) ((uint) num6 >> 1);
                                colorPtr2->R = num7;
                                colorPtr2->G = num7;
                                colorPtr2->B = LiquidRenderer.VISCOSITY_MASK[(int) liquidCachePtr9->VisibleType];
                                colorPtr2->A = num6;
                                var liquidCachePtr10 = liquidCachePtr9 - 1;
                                if (index2 != 2 && !liquidCachePtr10->HasVisibleLiquid &&
                                    (!liquidCachePtr10->IsSolid && !liquidCachePtr10->IsHalfBrick))
                                    *(colorPtr2 - 1) = *colorPtr2;
                            }
                            else
                            {
                                liquidDrawCachePtr2->IsVisible = false;
                                var index3 = liquidCachePtr9->IsSolid || liquidCachePtr9->IsHalfBrick ? 3 : 4;
                                var num2 = LiquidRenderer.WAVE_MASK_STRENGTH[index3];
                                var num3 = (byte) ((uint) num2 >> 1);
                                colorPtr2->R = num3;
                                colorPtr2->G = num3;
                ...
                var liquidCachePtr11 = liquidCachePtr1;
                for (var x = rectangle.X; x < rectangle.X + rectangle.Width; ++x)
                {
                    for (var y = rectangle.Y; y < rectangle.Y + rectangle.Height; ++y)
                    {
                        if (liquidCachePtr11->VisibleType == (byte) 1 && liquidCachePtr11->HasVisibleLiquid &&
                            Dust.lavaBubbles < 200)
                        {
                            if (this._random.Next(700) == 0)
                                Dust.NewDust(new Vector2((float) (x * 16), (float) (y * 16)), 16, 16, 35, 0.0f, 0.0f, 0,
                                    Color.White, 1f);
                            if (this._random.Next(350) == 0)
                            {
                                var index = Dust.NewDust(new Vector2((float) (x * 16), (float) (y * 16)), 16, 8, 35,
                                    0.0f, 0.0f, 50, Color.White, 1.5f);
            ...
            fixed (LiquidRenderer.LiquidDrawCache* liquidDrawCachePtr1 = &this._drawCache[0])
            {
                ...
                for (var x = drawArea.X; x < drawArea.X + drawArea.Width; ++x)
                {
                    for (var y = drawArea.Y; y < drawArea.Y + drawArea.Height; ++y)
                    {
                        ...
                        {
                            var sourceRectangle = liquidDrawCachePtr2->SourceRectangle;
                            if (liquidDrawCachePtr2->IsSurfaceLiquid)
                                sourceRectangle.Y = 1280;
                            else
                                sourceRectangle.Y += this._animationFrame * 80;
                            var liquidOffset = liquidDrawCachePtr2->LiquidOffset;
                            var val2 = liquidDrawCachePtr2->Opacity * (isBackgroundDraw
                                             ? 1f
                                             : LiquidRenderer.DEFAULT_OPACITY[(int) liquidDrawCachePtr2->Type]);
                            var index = (int) liquidDrawCachePtr2->Type;
                            switch (index)
                            {
                                case 0:
                                    index = waterStyle;
                            ...
                            Main.tileBatch.Draw(this._liquidTextures[index],
                                new Vector2((float) (x << 4), (float) (y << 4)) + drawOffset + liquidOffset,
                                new Rectangle?(sourceRectangle), vertices, Vector2.Zero, 1f, SpriteEffects.None);
                        }
        ...
        public bool HasFullWater(int x, int y)
        {
            x -= this._drawArea.X;
            y -= this._drawArea.Y;
            var index = x * this._drawArea.Height + y;
        ...
        public float GetVisibleLiquid(int x, int y)
        {
            ...
            if (x < 0 || x >= this._drawArea.Width || (y < 0 || y >= this._drawArea.Height))
                return 0.0f;
            var index = (x + 2) * (this._drawArea.Height + 4) + y + 2;
        ...
        public void Update(GameTime gameTime)
        {
            if (Main.gamePaused || !Main.hasFocus)
                return;
            var val2 = MathHelper.Clamp(Main.windSpeed * 80f, -20f, 20f);
            this._frameState += ((double) val2 >= 0.0 ? Math.Max(10f, val2) : Math.Min(-10f, val2)) *
        ...
        public void SetWaveMaskData(ref Texture2D texture)
        {
            ...
            {
                Console.WriteLine("WaveMaskData texture recreated. {0}x{1}", (object) this._drawArea.Height,
                    (object) this._drawArea.Width);
                if (texture != null)
                {
                    try
                    {
