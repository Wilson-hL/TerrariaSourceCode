...
namespace Terraria.GameContent.UI
{
    public class CustomCurrencyManager
    {
        ...
        public static void Initialize()
        {
            ...
            CustomCurrencyID.DefenderMedals =
                CustomCurrencyManager.RegisterCurrency((CustomCurrencySystem) new CustomCurrencySingleCoin(3817, 999L));
        }
            ...
            bool horizontal = false)
        {
            ...
            if (num1 > 0L)
                sb.Draw(Main.itemTexture[87],
                    Utils.CenteredRectangle(new Vector2(shopx + 70f, shopy + 60f), Main.itemTexture[87].Size() * 0.65f),
                    new Rectangle?(), Color.White);
            Utils.DrawBorderStringFourWay(sb, Main.fontMouseText, Lang.inter[66].Value, shopx, shopy + 40f,
        ...
        public static bool BuyItem(Player player, int price, int currencyIndex)
        {
            ...
            return currency.TryPurchasing(price, objArrayList, slotCoins, pointList1, pointList2, pointList3,
                pointList4);
        }
            ...
            for (int y = inventories[currentInventoryIndex].Length - 1; y >= 0; --y)
            {
                if (!slotsToIgnore[currentInventoryIndex].Contains(y) &&
                    (inventories[currentInventoryIndex][y].type == 0 ||
                     inventories[currentInventoryIndex][y].stack == 0))
