    ...
    public class EmoteBubble
    {
        ...
        private void Draw(SpriteBatch sb)
        {
            ...
            if (this.emote >= 0)
            {
                ...
                sb.Draw(texture2D, vector2,
                    new Rectangle?(texture2D.Frame(8, 35, this.emote * 2 % 8 + this.frame, 1 + this.emote / 4)),
                    Color.White, 0.0f, origin, 1f, effect, 0.0f);
            }
            else
            {
                if (this.emote != -1)
                ...
                sb.Draw(texture,
                    vector2 + new Vector2(effect.HasFlag((Enum) SpriteEffects.FlipHorizontally) ? 1f : -1f,
                        (float) (-rectangle.Height + 3)), new Rectangle?(), Color.White, 0.0f,
                    new Vector2((float) (texture.Width / 2), 0.0f), scale, effect, 0.0f);
            }
        }
        ...
        private Vector2 GetPosition(out SpriteEffects effect)
        {
            switch (this.anchor.type)
            {
                case WorldUIAnchor.AnchorType.Entity:
                    ...
                    return this.anchor.entity.Top +
                           new Vector2((float) (-this.anchor.entity.direction * this.anchor.entity.width) * 0.75f, 2f) -
                           Main.screenPosition;
                case WorldUIAnchor.AnchorType.Tile:
                    effect = SpriteEffects.None;
                    return this.anchor.pos - Main.screenPosition +
                           new Vector2(0.0f, (float) (-(double) this.anchor.size.Y / 2.0));
        ...
        private void ProbeWeather(List<int> list, Player plr)
        {
            ...
            if ((double) Main.cloudAlpha > 0.0)
            {
                ...
                if (plr.ZoneSnow)
                    list.Add(98);
                if ((double) plr.position.X < 4000.0 ||
                    (double) plr.position.X > (double) (Main.maxTilesX * 16 - 4000) &&
                    (double) plr.position.Y < Main.worldSurface / 16.0)
                    list.Add(97);
            }
            else
                list.Add(95);
        ...
        private void ProbeEvents(List<int> list)
        {
            ...
            if (Main.pumpkinMoon || (NPC.downedHalloweenKing || NPC.downedHalloweenTree) && !Main.dayTime)
                list.Add(20);
            if (Main.snowMoon ||
                (NPC.downedChristmasIceQueen || NPC.downedChristmasSantank || NPC.downedChristmasTree) && !Main.dayTime)
                list.Add(21);
        ...
        {
            if ((double) plr.Center.Y > (double) (Main.maxTilesY * 16 - 3200) || plr.onFire ||
                (((NPC) this.anchor.entity).onFire || plr.onFire2))
                list.Add(9);
            if (Main.rand.Next(2) == 0)
                list.Add(11);
        ...
        private void ProbeBiomes(List<int> list, Player plr)
        {
            if ((double) plr.position.Y / 16.0 < Main.worldSurface * 0.45)
                ...
                list.Add(32);
            else if ((double) plr.position.Y / 16.0 < Main.worldSurface &&
                     ((double) plr.position.X < 4000.0 ||
                      (double) plr.position.X > (double) (16 * (Main.maxTilesX - 250))))
                list.Add(29);
