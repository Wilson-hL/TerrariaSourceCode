        ...
        public bool InRange(Vector2 target, float tileRangeX, float tileRangeY)
        {
            switch (this.type)
            {
                case WorldUIAnchor.AnchorType.Entity:
                    if ((double) Math.Abs(target.X - this.entity.Center.X) <=
                        (double) tileRangeX * 16.0 + (double) this.entity.width / 2.0)
                        return (double) Math.Abs(target.Y - this.entity.Center.Y) <=
                               (double) tileRangeY * 16.0 + (double) this.entity.height / 2.0;
                    return false;
                case WorldUIAnchor.AnchorType.Tile:
                    if ((double) Math.Abs(target.X - this.pos.X) <=
                        (double) tileRangeX * 16.0 + (double) this.size.X / 2.0)
                        return (double) Math.Abs(target.Y - this.pos.Y) <=
                               (double) tileRangeY * 16.0 + (double) this.size.Y / 2.0;
                    return false;
