...
namespace Terraria.GameContent.Skies
{
    public class PartySky : CustomSky
    {
            ...
            for (var i = 0; i < this._balloons.Length; ++i)
            {
                if (this._balloons[i].Active)
                {
                    ...
                    if ((double) this._balloons[i].Position.Y < 300.0)
                    {
                        ...
                        {
                            this.ResetBalloon(i);
                            this._balloons[i].Position =
                                new Vector2((float) (this._random.Next(0, Main.maxTilesX) * 16),
                                    (float) (Main.worldSurface * 16.0 + 1600.0));
                            if (this._random.Next(30) == 0)
                                this._balloons[i].Texture = this._textures[2];
                        }
                        else
            ...
            for (var index = num1; index < num2; ++index)
            {
                if (this._balloons[index].Active)
                {
                    ...
                    if (rectangle.Contains((int) position.X, (int) position.Y))
                        spriteBatch.Draw(this._balloons[index].Texture, position,
                            new Rectangle?(this._balloons[index].GetSourceRectangle()), color * this._opacity, 0.0f,
                            Vector2.Zero, vector2_2.X * 2f, SpriteEffects.None, 0.0f);
                }
            }
        }
        ...
        private struct Balloon
        {
            ...
            {
                return new Rectangle(this.FrameWidth * this.Variant, this._frameCounter / 14 * this.FrameHeight,
