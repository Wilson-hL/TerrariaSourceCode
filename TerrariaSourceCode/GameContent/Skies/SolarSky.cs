        ...
        public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
        {
            if ((double) maxDepth >= 3.40282346638529E+38 && (double) minDepth < 3.40282346638529E+38)
            {
                spriteBatch.Draw(Main.blackTileTexture, new Rectangle(0, 0, Main.screenWidth, Main.screenHeight),
                    Color.Black * this._fadeOpacity);
                spriteBatch.Draw(this._bgTexture,
                    new Rectangle(0,
                        Math.Max(0,
                            (int) ((Main.worldSurface * 16.0 - (double) Main.screenPosition.Y - 2400.0) *
                ...
                spriteBatch.Draw(this._planetTexture, vector2_1 + new Vector2(-200f, -200f) + vector2_2,
                    ...
                    new Vector2((float) (this._planetTexture.Width >> 1), (float) (this._planetTexture.Height >> 1)),
            ...
            for (int index = num1; index < num2; ++index)
            {
                ...
                if (rectangle.Contains((int) position.X, (int) position.Y))
                    spriteBatch.Draw(this._meteorTexture, position,
                        new Rectangle?(new Rectangle(0, num4 * (this._meteorTexture.Height / 4),
                            this._meteorTexture.Width, this._meteorTexture.Height / 4)),
                        Color.White * num3 * this._fadeOpacity, 0.0f, Vector2.Zero,
                        vector2_1.X * 5f * this._meteors[index].Scale, SpriteEffects.None, 0.0f);
            }
        }
        ...
        internal override void Activate(Vector2 position, params object[] args)
        {
            ...
            for (int index = 0; index < this._meteors.Length; ++index)
            {
                ...
                this._meteors[index].Position.Y =
                    (float) ((double) this._random.NextFloat() * -(Main.worldSurface * 16.0 + 10000.0) - 10000.0);
                this._meteors[index].Depth = this._random.Next(3) == 0
                    ? (float) ((double) this._random.NextFloat() * 5.0 + 4.80000019073486)
                    : (float) ((double) this._random.NextFloat() * 3.0 + 1.79999995231628);
