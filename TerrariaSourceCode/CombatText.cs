...
namespace Terraria
{
    public class CombatText
    {
        ...
        public static int NewText(Rectangle location, Color color, string text, bool dramatic = false, bool dot = false)
        {
            ...
            for (var index1 = 0; index1 < 100; ++index1)
            {
                if (!Main.combatText[index1].active)
                {
                    ...
                    Main.combatText[index1].position.X =
                        (float) ((double) location.X + (double) location.Width * 0.5 - (double) vector2.X * 0.5);
                    Main.combatText[index1].position.Y =
                        (float) ((double) location.Y + (double) location.Height * 0.25 - (double) vector2.Y * 0.5);
                    Main.combatText[index1].position.X += (float) Main.rand.Next(-(int) ((double) location.Width * 0.5),
                        (int) ((double) location.Width * 0.5) + 1);
                    Main.combatText[index1].position.Y += (float) Main.rand.Next(
                        -(int) ((double) location.Height * 0.5), (int) ((double) location.Height * 0.5) + 1);
                    ...
                    if ((double) Main.player[Main.myPlayer].gravDir == -1.0)
                    {
                        ...
                        Main.combatText[index1].position.Y =
                            (float) ((double) location.Y + (double) location.Height * 0.75 + (double) vector2.Y * 0.5);
                    }
        ...
        {
            get { return Main.UIScale / (Main.GameViewMatrix.Zoom.X / Main.ForcedMinimumZoom); }
        }
