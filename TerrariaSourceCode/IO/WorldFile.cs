    ...
    public class WorldFile
    {
            ...
            using (MemoryStream memoryStream = new MemoryStream(FileUtilities.ReadAllBytes(Main.worldPathName, flag)))
            {
                using (BinaryReader binaryReader = new BinaryReader((Stream) memoryStream))
                {
                    try
                    {
                        ...
                        if (num1 < 141)
                            Main.ActiveWorldFileData.CreationTime =
                                loadFromCloud ? DateTime.Now : File.GetCreationTime(Main.worldPathName);
                        binaryReader.Close();
                        memoryStream.Close();
                        if (num2 != 0)
                        ...
                        while (Liquid.numLiquid > 0 && num3 < 100000)
                        {
                            ...
                            Main.statusText = Lang.gen[27].Value + " " +
                                              (object) (int) ((double) num6 * 100.0 / 2.0 + 50.0) + "%";
                            Liquid.UpdateLiquid();
                        }
                ...
                for (int index2 = 0; index2 < Main.maxTilesY; ++index2)
                {
                    ...
                    if (tile.active())
                    {
                        ...
                        if (tile.type == (ushort) sbyte.MaxValue)
                            tile.active(false);
                        if (versionNumber < 72 && (tile.type == (ushort) 35 || tile.type == (ushort) 36 ||
                                                   (tile.type == (ushort) 170 || tile.type == (ushort) 171) ||
                                                   tile.type == (ushort) 172))
                        {
        ...
        public static void SaveWorld_Version2(BinaryWriter writer)
        {
            int[] pointers = new int[10]
            {
                ...
                WorldFile.SaveNPCs(writer), WorldFile.SaveTileEntities(writer),
                WorldFile.SaveWeightedPressurePlates(writer), WorldFile.SaveTownManager(writer), 0
            };
                    ...
                    if (tile.active())
                    {
                        ...
                        if (tile.type == (ushort) sbyte.MaxValue)
                        {
                            ...
                            if (!tile.active())
                            {
                                flag = false;
                                if (Main.netMode != 0)
                                    NetMessage.SendData(17, -1, -1, (NetworkText) null, 0, (float) i, (float) j, 0.0f,
                                        0, 0, 0);
                            }
                        }
                    }
                    ...
                    if (flag)
                    {
                        ...
                        if (tile.type == (ushort) sbyte.MaxValue)
                        {
                            ...
                            if (!tile.active() && Main.netMode != 0)
                                NetMessage.SendData(17, -1, -1, (NetworkText) null, 0, (float) i, (float) j, 0.0f, 0, 0,
                                    0);
                        }
        ...
        {
            return !reader.ReadBoolean() || reader.ReadString() != Main.worldName || reader.ReadInt32() != Main.worldID
                ? 6
                : 0;
        }
        ...
        private static void LoadTileEntities(BinaryReader reader)
        {
            ...
            foreach (KeyValuePair<Point16, TileEntity> keyValuePair in TileEntity.ByPosition)
            {
                ...
                {
                    if (keyValuePair.Value.type == (byte) 0 &&
                        !TETrainingDummy.ValidTile((int) keyValuePair.Value.Position.X,
                            (int) keyValuePair.Value.Position.Y))
                        point16List.Add(keyValuePair.Value.Position);
                    if (keyValuePair.Value.type == (byte) 2 &&
                        !TELogicSensor.ValidTile((int) keyValuePair.Value.Position.X,
                            (int) keyValuePair.Value.Position.Y))
                        point16List.Add(keyValuePair.Value.Position);
                    if (keyValuePair.Value.type == (byte) 1 &&
                        !TEItemFrame.ValidTile((int) keyValuePair.Value.Position.X,
                            (int) keyValuePair.Value.Position.Y))
                        point16List.Add(keyValuePair.Value.Position);
                }
            }
