...
namespace Terraria
{
    public class ItemText
    {
        ...
        {
            get { return Main.UIScale / Main.GameViewMatrix.Zoom.X; }
        }
        ...
        public static void NewText(Item newItem, int stack, bool noStack = false, bool longText = false)
        {
            ...
            for (int index = 0; index < 20; ++index)
            {
                    ...
                    (!Main.itemText[index].NoStack && !noStack))
                {
                    ...
                    Main.itemText[index].position.X =
                        (float) ((double) newItem.position.X + (double) newItem.width * 0.5 - (double) vector2.X * 0.5);
                    Main.itemText[index].position.Y =
                        (float) ((double) newItem.position.Y + (double) newItem.height * 0.25 -
                                 (double) vector2.Y * 0.5);
            ...
            Main.itemText[index1].position.X =
                (float) ((double) newItem.position.X + (double) newItem.width * 0.5 - (double) vector2_1.X * 0.5);
            Main.itemText[index1].position.Y =
                (float) ((double) newItem.position.Y + (double) newItem.height * 0.25 - (double) vector2_1.Y * 0.5);
            ...
            for (int index = 0; index < 20; ++index)
            {
                if (Main.itemText[index].active && index != whoAmI)
                {
                    ...
                    if (rectangle1.Intersects(rectangle2) &&
                        ((double) this.position.Y < (double) Main.itemText[index].position.Y ||
