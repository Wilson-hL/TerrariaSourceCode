...
namespace Terraria.GameInput
{
    public class PlayerInputProfile
    {
        ...
        {
            get { return this.HotbarRadialHoldTimeRequired != -1; }
        }
        ...
        public bool Load(Dictionary<string, object> dict)
        {
            ...
            if (dict.TryGetValue("Mouse And Keyboard", out obj))
                this.InputModes[InputMode.Keyboard].ReadPreferences(
                    (Dictionary<string, List<string>>) JsonConvert.DeserializeObject<Dictionary<string, List<string>>>(
                        ((object) (JObject) obj).ToString()));
            if (dict.TryGetValue("Gamepad", out obj))
                this.InputModes[InputMode.XBoxGamepad].ReadPreferences(
                    (Dictionary<string, List<string>>) JsonConvert.DeserializeObject<Dictionary<string, List<string>>>(
                        ((object) (JObject) obj).ToString()));
            if (dict.TryGetValue("Mouse And Keyboard UI", out obj))
                this.InputModes[InputMode.KeyboardUI].ReadPreferences(
                    (Dictionary<string, List<string>>) JsonConvert.DeserializeObject<Dictionary<string, List<string>>>(
                        ((object) (JObject) obj).ToString()));
            if (dict.TryGetValue("Gamepad UI", out obj))
                this.InputModes[InputMode.XBoxGamepadUI].ReadPreferences(
                    (Dictionary<string, List<string>>) JsonConvert.DeserializeObject<Dictionary<string, List<string>>>(
                        ((object) (JObject) obj).ToString()));
            if (num < 190)
            {
                this.InputModes[InputMode.Keyboard].KeyStatus["ViewZoomIn"] = new List<string>();
                this.InputModes[InputMode.Keyboard].KeyStatus["ViewZoomIn"].AddRange(
                    (IEnumerable<string>) PlayerInput.OriginalProfiles["Redigit's Pick"].InputModes[InputMode.Keyboard]
                        .KeyStatus["ViewZoomIn"]);
                this.InputModes[InputMode.Keyboard].KeyStatus["ViewZoomOut"] = new List<string>();
                this.InputModes[InputMode.Keyboard].KeyStatus["ViewZoomOut"].AddRange(
                    (IEnumerable<string>) PlayerInput.OriginalProfiles["Redigit's Pick"].InputModes[InputMode.Keyboard]
                        .KeyStatus["ViewZoomOut"]);
            }
        ...
        public void ConditionalAddProfile(Dictionary<string, object> dicttouse, string k, InputMode nm,
        ...
        public void ConditionalAdd(Dictionary<string, object> dicttouse, string a, object b,
        ...
        public bool UsingDpadHotbar()
        {
            if (this.InputModes[InputMode.XBoxGamepad].KeyStatus["DpadRadial1"].Contains(Buttons.DPadUp.ToString()) &&
                this.InputModes[InputMode.XBoxGamepad].KeyStatus["DpadRadial2"]
                    .Contains(Buttons.DPadRight.ToString()) &&
                (this.InputModes[InputMode.XBoxGamepad].KeyStatus["DpadRadial3"]
                     .Contains(Buttons.DPadDown.ToString()) &&
                 this.InputModes[InputMode.XBoxGamepad].KeyStatus["DpadRadial4"].Contains(Buttons.DPadLeft.ToString())
                ) && (this.InputModes[InputMode.XBoxGamepadUI].KeyStatus["DpadRadial1"]
                          .Contains(Buttons.DPadUp.ToString()) &&
                      this.InputModes[InputMode.XBoxGamepadUI].KeyStatus["DpadRadial2"]
                          .Contains(Buttons.DPadRight.ToString()) && this.InputModes[InputMode.XBoxGamepadUI]
                          .KeyStatus["DpadRadial3"].Contains(Buttons.DPadDown.ToString())))
                return this.InputModes[InputMode.XBoxGamepadUI].KeyStatus["DpadRadial4"]
        ...
        public bool UsingDpadMovekeys()
        {
            if (this.InputModes[InputMode.XBoxGamepad].KeyStatus["DpadSnap1"].Contains(Buttons.DPadUp.ToString()) &&
                this.InputModes[InputMode.XBoxGamepad].KeyStatus["DpadSnap2"].Contains(Buttons.DPadRight.ToString()) &&
                (this.InputModes[InputMode.XBoxGamepad].KeyStatus["DpadSnap3"].Contains(Buttons.DPadDown.ToString()) &&
                 this.InputModes[InputMode.XBoxGamepad].KeyStatus["DpadSnap4"].Contains(Buttons.DPadLeft.ToString())) &&
                (this.InputModes[InputMode.XBoxGamepadUI].KeyStatus["DpadSnap1"].Contains(Buttons.DPadUp.ToString()) &&
                 this.InputModes[InputMode.XBoxGamepadUI].KeyStatus["DpadSnap2"]
                     .Contains(Buttons.DPadRight.ToString()) && this.InputModes[InputMode.XBoxGamepadUI]
                     .KeyStatus["DpadSnap3"].Contains(Buttons.DPadDown.ToString())))
                return this.InputModes[InputMode.XBoxGamepadUI].KeyStatus["DpadSnap4"]
                    .Contains(Buttons.DPadLeft.ToString());
            return false;
        }
    }
