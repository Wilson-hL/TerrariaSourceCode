...
namespace Terraria
{
    public static class DelegateMethods
    {
        ...
        {
            return Main.hslToRgb(DelegateMethods.v3_1.X, DelegateMethods.v3_1.Y,
                (float) (0.150000005960464 + 0.850000023841858 * (double) percent));
        }

        public static Color ColorLerp_HSL_O(float percent)
        {
            return Color.Lerp(Color.White,
                Main.hslToRgb(DelegateMethods.v3_1.X, DelegateMethods.v3_1.Y, DelegateMethods.v3_1.Z), percent);
        }
        ...
        public static bool CastLightOpen(int x, int y)
        {
            if (x < 0 || x >= Main.maxTilesX || (y < 0 || y >= Main.maxTilesY) || Main.tile[x, y] == null)
                return false;
            if (!Main.tile[x, y].active() || Main.tile[x, y].inActive() ||
                (Main.tileSolidTop[(int) Main.tile[x, y].type] || !Main.tileSolid[(int) Main.tile[x, y].type]))
                Lighting.AddLight(x, y, DelegateMethods.v3_1.X, DelegateMethods.v3_1.Y, DelegateMethods.v3_1.Z);
            return true;
        }
        ...
        public static bool NotDoorStand(int x, int y)
        {
            ...
            if (Main.tile[x, y].frameX >= (short) 18)
                return Main.tile[x, y].frameX < (short) 54;
            return false;
        }
        ...
        {
            return WorldGen.InWorld(x, y, 1) && Main.tile[x, y] != null &&
                   (!Main.tile[x, y].active() || !TileID.Sets.AvoidedByNPCs[(int) Main.tile[x, y].type]);
        }

        public static void RainbowLaserDraw(int stage, Vector2 currentPosition, float distanceLeft, Rectangle lastFrame,
        ...
        public static void TurretLaserDraw(int stage, Vector2 currentPosition, float distanceLeft, Rectangle lastFrame,
            out float distCovered, out Rectangle frame, out Vector2 origin, out Color color)
        {
            color = DelegateMethods.c_1;
            switch (stage)
        ...
        public static void LightningLaserDraw(int stage, Vector2 currentPosition, float distanceLeft,
            Rectangle lastFrame, out float distCovered, out Rectangle frame, out Vector2 origin, out Color color)
        {
