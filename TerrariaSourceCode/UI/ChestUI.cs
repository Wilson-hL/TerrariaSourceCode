    ...
    public class ChestUI
    {
        ...
        public static void Draw(SpriteBatch spritebatch)
        {
            if (Main.player[Main.myPlayer].chest != -1 && !Main.recBigList)
            {
                ...
                if (Utils.FloatIntersect((float) Main.mouseX, (float) Main.mouseY, 0.0f, 0.0f, 73f,
                    (float) Main.instance.invBottom, 560f * Main.inventoryScale, 224f * Main.inventoryScale))
        ...
        private static void DrawName(SpriteBatch spritebatch)
        {
            ...
            var baseColor = Color.White *
                              (float) (1.0 - ((double) byte.MaxValue - (double) Main.mouseTextColor) /
                                       (double) byte.MaxValue * 0.5);
            baseColor.A = byte.MaxValue;
            int lineAmount;
            Utils.WordwrapString(text, Main.fontMouseText, 200, 1, out lineAmount);
            ++lineAmount;
            for (var index = 0; index < lineAmount; ++index)
                ChatManager.DrawColorCodedStringWithShadow(spritebatch, Main.fontMouseText, text,
                    new Vector2(504f, (float) (Main.instance.invBottom + index * 26)), baseColor, 0.0f, Vector2.Zero,
                    Vector2.One, -1f, 1.5f);
        }
        ...
        private static void DrawButton(SpriteBatch spriteBatch, int ID, int X, int Y)
        {
            ...
            {
                Y += ID * 26;
                var num = ChestUI.ButtonScale[ID];
                var text = "";
                switch (ID)
                {
                    case 0:
                ...
                var vector2 = Main.fontMouseText.MeasureString(text);
                var color = new Color((int) Main.mouseTextColor, (int) Main.mouseTextColor, (int) Main.mouseTextColor,
                                  (int) Main.mouseTextColor) * num;
                var baseColor = Color.White * 0.97f *
                                  (float) (1.0 - ((double) byte.MaxValue - (double) Main.mouseTextColor) /
                                           (double) byte.MaxValue * 0.5);
                baseColor.A = byte.MaxValue;
                X += (int) ((double) vector2.X * (double) num / 2.0);
                ChatManager.DrawColorCodedStringWithShadow(spriteBatch, Main.fontMouseText, text,
                    new Vector2((float) X, (float) Y), baseColor, 0.0f, vector2 / 2f, new Vector2(num), -1f, 1.5f);
                vector2 *= num;
                switch (ID)
                {
                    case 0:
                        UILinkPointNavigator.SetPosition(500,
                            new Vector2(
                                (float) X - (float) ((double) vector2.X * (double) num / 2.0 * 0.800000011920929),
                                (float) Y));
                        break;
                    case 1:
                        UILinkPointNavigator.SetPosition(501,
                            new Vector2(
                                (float) X - (float) ((double) vector2.X * (double) num / 2.0 * 0.800000011920929),
                                (float) Y));
                        break;
                    case 2:
                        UILinkPointNavigator.SetPosition(502,
                            new Vector2(
                                (float) X - (float) ((double) vector2.X * (double) num / 2.0 * 0.800000011920929),
                                (float) Y));
                        break;
                    case 3:
                        UILinkPointNavigator.SetPosition(503,
                            new Vector2(
                                (float) X - (float) ((double) vector2.X * (double) num / 2.0 * 0.800000011920929),
                                (float) Y));
                        break;
                    case 4:
                        UILinkPointNavigator.SetPosition(505,
                            new Vector2(
                                (float) X - (float) ((double) vector2.X * (double) num / 2.0 * 0.800000011920929),
                                (float) Y));
                        break;
                ...
                if (!Utils.FloatIntersect((float) Main.mouseX, (float) Main.mouseY, 0.0f, 0.0f,
                    (float) X - vector2.X / 2f, (float) Y - vector2.Y / 2f, vector2.X, vector2.Y))
                {
                    ChestUI.UpdateHover(ID, false);
                }
                else
        ...
        private static void DrawSlots(SpriteBatch spriteBatch)
        {
            ...
            for (var index1 = 0; index1 < 10; ++index1)
            {
                ...
                {
                    var num1 = (int) (73.0 + (double) (index1 * 56) * (double) Main.inventoryScale);
                    var num2 = (int) ((double) Main.instance.invBottom +
                                      (double) (index2 * 56) * (double) Main.inventoryScale);
                    var slot = index1 + index2 * 10;
                    var color = new Color(100, 100, 100, 100);
                    if (Utils.FloatIntersect((float) Main.mouseX, (float) Main.mouseY, 0.0f, 0.0f, (float) num1,
                            (float) num2, (float) Main.inventoryBackTexture.Width * Main.inventoryScale,
                            (float) Main.inventoryBackTexture.Height * Main.inventoryScale) &&
                        !PlayerInput.IgnoreMouseInterface)
                    {
                        player.mouseInterface = true;
            ...
            if (player.chest > -1)
            {
                ...
                for (var index = 0; index < 40; ++index)
                {
                    if (chest.item[index].type > 0)
                    {
                        ...
                        if (Main.netMode == 1)
                            NetMessage.SendData(32, -1, -1, (NetworkText) null, player.chest, (float) index, 0.0f, 0.0f,
                                0, 0, 0);
                    }
                }
            }
            else if (player.chest == -3)
            {
                for (var index = 0; index < 40; ++index)
                {
                    if (player.bank2.item[index].type > 0)
                    {
                        ...
                        player.bank2.item[index] =
                            player.GetItem(Main.myPlayer, player.bank2.item[index], false, false);
                    }
                }
            }
            else if (player.chest == -4)
            {
                for (var index = 0; index < 40; ++index)
                {
                    if (player.bank3.item[index].type > 0)
                    {
                        ...
                        player.bank3.item[index] =
                            player.GetItem(Main.myPlayer, player.bank3.item[index], false, false);
                    }
                }
            }
            else
        ...
        public static void DepositAll()
        {
            ...
            for (var index1 = 49; index1 >= 10; --index1)
            {
                if (player.inventory[index1].stack > 0 && player.inventory[index1].type > 0 &&
                    !player.inventory[index1].favorited)
                {
                    if (player.inventory[index1].maxStack > 1)
                    {
                        for (var index2 = 0; index2 < 40; ++index2)
                        {
                            ...
                            {
                                var chest = Main.chest[player.chest];
                                if (chest.item[index2].stack < chest.item[index2].maxStack &&
                                    player.inventory[index1].IsTheSameAs(chest.item[index2]))
                                {
                                    var num = player.inventory[index1].stack;
                                    if (player.inventory[index1].stack + chest.item[index2].stack >
                                        chest.item[index2].maxStack)
                                    ...
                                    if (player.inventory[index1].stack <= 0)
                                    {
                                        ...
                                        if (Main.netMode == 1)
                                        {
                                            NetMessage.SendData(32, -1, -1, (NetworkText) null, player.chest,
                            ...
                            {
                                if (player.bank2.item[index2].stack < player.bank2.item[index2].maxStack &&
                                    player.inventory[index1].IsTheSameAs(player.bank2.item[index2]))
                                {
                                    var num = player.inventory[index1].stack;
                                    if (player.inventory[index1].stack + player.bank2.item[index2].stack >
                                        player.bank2.item[index2].maxStack)
                            ...
                            {
                                if (player.bank3.item[index2].stack < player.bank3.item[index2].maxStack &&
                                    player.inventory[index1].IsTheSameAs(player.bank3.item[index2]))
                                {
                                    var num = player.inventory[index1].stack;
                                    if (player.inventory[index1].stack + player.bank3.item[index2].stack >
                                        player.bank3.item[index2].maxStack)
                            ...
                            else if (player.bank.item[index2].stack < player.bank.item[index2].maxStack &&
                                     player.inventory[index1].IsTheSameAs(player.bank.item[index2]))
                            {
                                var num = player.inventory[index1].stack;
                                if (player.inventory[index1].stack + player.bank.item[index2].stack >
                                    player.bank.item[index2].maxStack)
        ...
        public static void QuickStack()
        {
            ...
            var flagArray = new bool[objArray.Length];
            for (var index = 0; index < 40; ++index)
            {
                if (objArray[index].type > 0 && objArray[index].stack > 0 && objArray[index].maxStack > 1 &&
                    (objArray[index].type < 71 || objArray[index].type > 74))
                {
                    intList2.Add(index);
            ...
            if (player.chest <= -2)
                num1 += 4;
            for (var key = 0; key < num1; ++key)
            {
                if (intList1.Contains(inventory[key].netID) && !inventory[key].favorited)
                    dictionary.Add(key, inventory[key].netID);
            }

            for (var index1 = 0; index1 < intList2.Count; ++index1)
            {
                var index2 = intList2[index1];
                var netId = objArray[index2].netID;
                foreach (var keyValuePair in dictionary)
                {
                    if (keyValuePair.Value == netId && inventory[keyValuePair.Key].netID == netId)
                    {
                        var num2 = inventory[keyValuePair.Key].stack;
                        var num3 = objArray[index2].maxStack - objArray[index2].stack;
                        if (num3 != 0)
                        {
                            if (num2 > num3)
                                num2 = num3;
            ...
            foreach (var key in intList4)
                dictionary.Remove(key);
            for (var index1 = 0; index1 < intList3.Count; ++index1)
            {
                var index2 = intList3[index1];
                var flag = true;
                var netId = objArray[index2].netID;
                foreach (var keyValuePair in dictionary)
                {
                    if (keyValuePair.Value == netId && inventory[keyValuePair.Key].netID == netId ||
                        flag && inventory[keyValuePair.Key].stack > 0)
                    {
                        Main.PlaySound(7, -1, -1, 1, 1f, 0.0f);
                        if (flag)
                        {
            ...
            if (Main.netMode == 1 && player.chest >= 0)
            {
                for (var index = 0; index < flagArray.Length; ++index)
                    NetMessage.SendData(32, -1, -1, (NetworkText) null, player.chest, (float) index, 0.0f, 0.0f, 0, 0,
                        0);
            }
        ...
        public static void Restock()
        {
            ...
            var intList1 = new List<int>();
            var intList2 = new List<int>();
            for (var index = 57; index >= 0; --index)
            {
                if ((index < 50 || index >= 54) && (inventory[index].type < 71 || inventory[index].type > 74))
                {
                    if (inventory[index].stack > 0 && inventory[index].maxStack > 1 &&
                        inventory[index].prefix == (byte) 0)
                    {
            ...
            var flag1 = false;
            for (var index1 = 0; index1 < objArray.Length; ++index1)
            {
                if (objArray[index1].stack >= 1 && objArray[index1].prefix == (byte) 0 &&
                    intSet.Contains(objArray[index1].netID))
                {
                    var flag2 = false;
                    for (var index2 = 0; index2 < intList1.Count; ++index2)
                    {
                        var slot = intList1[index2];
                        var context = 0;
                        if (slot >= 50)
                            context = 2;
                        if (inventory[slot].netID == objArray[index1].netID &&
                            ItemSlot.PickItemMovementAction(inventory, context, slot, objArray[index1]) != -1)
                        {
                            var num = objArray[index1].stack;
                            ...
                            if (inventory[slot].stack == inventory[slot].maxStack)
                            {
                                if (Main.netMode == 1 && Main.player[Main.myPlayer].chest > -1)
                                    NetMessage.SendData(32, -1, -1, (NetworkText) null,
                                        Main.player[Main.myPlayer].chest, (float) index1, 0.0f, 0.0f, 0, 0, 0);
                    ...
                    {
                        for (var index2 = 0; index2 < intList2.Count; ++index2)
                        {
                            var context = 0;
                            if (intList2[index2] >= 50)
                                context = 2;
                            if (ItemSlot.PickItemMovementAction(inventory, context, intList2[index2],
                                    objArray[index1]) != -1)
                            {
        ...
        public static void MoveCoins(Item[] pInv, Item[] cInv)
        {
            ...
            for (var index = 0; index < cInv.Length; ++index)
            {
                numArray2[index] = -1;
            ...
            if (!flag)
                return;
            Main.PlaySound(7, -1, -1, 1, 1f, 0.0f);
            for (var index = 0; index < pInv.Length; ++index)
            {
                ...
                for (var index = 0; index < 40; ++index)
                {
                    if (objArray[index].stack < objArray[index].maxStack && I.IsTheSameAs(objArray[index]))
                    {
                        ...
                        if (I.stack <= 0)
                        {
                            ...
                            if (flag1)
                            {
                                NetMessage.SendData(32, -1, -1, (NetworkText) null, player.chest, (float) index, 0.0f,
                                    0.0f, 0, 0, 0);
                                break;
                            }
        ...
        public static bool TryPlacingInPlayer(int slot, bool justCheck)
        {
            ...
            var obj = objArray[slot];
            var flag2 = false;
            if (obj.maxStack > 1)
            {
                for (var index = 49; index >= 0; --index)
                {
