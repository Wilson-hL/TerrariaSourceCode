...
namespace Terraria
{
    public static class TimeLogger
    {
            ...
            for (int index = 0; index < TimeLogger.detailedDrawTimes.Length; ++index)
            {
                if (string.IsNullOrEmpty(TimeLogger.detailedDrawTimes[index].logText))
                    TimeLogger.detailedDrawTimes[index].logText =
                        string.Format("Unnamed detailed draw #{0}", (object) index);
            }
        }
        ...
        public static void NewDrawFrame()
        {
            ...
            if (TimeLogger.startLoggingNextFrame)
            {
                ...
                try
                {
                    TimeLogger.logWriter =
                        new StreamWriter((Stream) new GZipStream((Stream) new FileStream(path, FileMode.Create),
            ...
            if (TimeLogger.currentlyLogging)
                TimeLogger.logBuilder.AppendLine(string.Format("Start of Frame #{0}",
                    (object) TimeLogger.currentFrame));
        ...
        private static void UpdateTime(TimeLogger.TimeLogData[] times, int type, double time)
        {
            ...
            if (TimeLogger.currentFrame == 0)
                return;
            TimeLogger.logBuilder.AppendLine(string.Format("    {0} : {1:F4}ms {2}", (object) times[type].logText,
                (object) time, flag ? (object) " - New Maximum" : (object) string.Empty));
        }
        ...
        public static void MapDrawTime(double timeElapsed)
        {
            if (!TimeLogger.currentlyLogging)
                return;
            TimeLogger.logBuilder.AppendLine(
                string.Format("Full Screen Map Render Time : {0:F4}", (object) timeElapsed));
        }
