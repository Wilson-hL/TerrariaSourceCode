    ...
    public class Netplay
    {
        ...
        public static void TcpClientLoop(object threadContext)
        {
            ...
            while (flag)
            {
                ...
                try
                {
                    Netplay.Connection.Socket.Connect((RemoteAddress) new TcpAddress(Netplay.ServerIP,
                        Netplay.ListenPort));
                    flag = false;
                }
                catch
                ...
                while (!Netplay.disconnect)
                {
                    if (Netplay.Connection.Socket.IsConnected())
                    {
                        ...
                        else if (Netplay.Connection.State == 5 && WorldGen.worldCleared)
                        {
                            ...
                            NetMessage.SendData(8, -1, -1, (NetworkText) null, Main.player[Main.myPlayer].SpawnX,
                                (float) Main.player[Main.myPlayer].SpawnY, 0.0f, 0.0f, 0, 0, 0);
                        }

                        if (Netplay.Connection.State == 6 && num1 != Netplay.Connection.State)
                            Main.statusText = Language.GetTextValue("Net.RequestingTileData");
                        if (!Netplay.Connection.IsReading && !Netplay.disconnect &&
                            Netplay.Connection.Socket.IsDataAvailable())
                        {
                            Netplay.Connection.IsReading = true;
                            Netplay.Connection.Socket.AsyncReceive(Netplay.Connection.ReadBuffer, 0,
                                Netplay.Connection.ReadBuffer.Length,
                                new SocketReceiveCallback(Netplay.Connection.ClientReadCallBack), (object) null);
                        }
                        ...
                        if (Netplay.Connection.StatusMax > 0 && Netplay.Connection.StatusText != "")
                        {
                            ...
                            {
                                Main.statusText = Language.GetTextValue("Net.StatusComplete",
                                ...
                                Main.statusText = Netplay.Connection.StatusText + ": " +
        ...
        public static void ServerLoop(object threadContext)
        {
            ...
            while (!Netplay.disconnect)
            {
                ...
                for (int index = 0; index < 256; ++index)
                {
                    ...
                    else if (Netplay.Clients[index].IsConnected())
                    {
                        ...
                        if (!Netplay.Clients[index].IsReading)
                        {
                            try
                            {
                                if (Netplay.Clients[index].Socket.IsDataAvailable())
                                {
                                    ...
                                    Netplay.Clients[index].Socket.AsyncReceive(Netplay.Clients[index].ReadBuffer, 0,
                                        Netplay.Clients[index].ReadBuffer.Length,
                                        new SocketReceiveCallback(Netplay.Clients[index].ServerReadCallBack),
                                        (object) null);
                                }
                            }
                            catch
                        ...
                        if (Netplay.Clients[index].StatusMax > 0 && Netplay.Clients[index].StatusText2 != "")
                        {
                            ...
                            {
                                Netplay.Clients[index].StatusText = Language.GetTextValue("Net.ClientStatusComplete",
                                    (object) Netplay.Clients[index].Socket.GetRemoteAddress(),
                                    (object) Netplay.Clients[index].Name, (object) Netplay.Clients[index].StatusText2);
                                Netplay.Clients[index].StatusText2 = "";
                                Netplay.Clients[index].StatusMax = 0;
                                Netplay.Clients[index].StatusCount = 0;
                            }
                            else
                                Netplay.Clients[index].StatusText =
                                    "(" + (object) Netplay.Clients[index].Socket.GetRemoteAddress() + ") " +
                                    Netplay.Clients[index].Name + " " + Netplay.Clients[index].StatusText2 + ": " +
                                    (object) (int) ((double) Netplay.Clients[index].StatusCount /
                                                    (double) Netplay.Clients[index].StatusMax * 100.0) + "%";
                        }
                        else if (Netplay.Clients[index].State == 0)
                            Netplay.Clients[index].StatusText = Language.GetTextValue("Net.ClientConnecting",
                                (object) string.Format("({0}) {1}",
                                    (object) Netplay.Clients[index].Socket.GetRemoteAddress(),
                                    (object) Netplay.Clients[index].Name));
                        else if (Netplay.Clients[index].State == 1)
                            Netplay.Clients[index].StatusText = Language.GetTextValue("Net.ClientSendingData",
                                (object) Netplay.Clients[index].Socket.GetRemoteAddress(),
                                (object) Netplay.Clients[index].Name);
                        else if (Netplay.Clients[index].State == 2)
                            Netplay.Clients[index].StatusText = Language.GetTextValue("Net.ClientRequestedWorldInfo",
                                (object) Netplay.Clients[index].Socket.GetRemoteAddress(),
                                (object) Netplay.Clients[index].Name);
                        else if (Netplay.Clients[index].State != 3)
                        {
                            if (Netplay.Clients[index].State == 10)
                            {
                                try
                                {
                                    Netplay.Clients[index].StatusText = Language.GetTextValue("Net.ClientPlaying",
                                        (object) Netplay.Clients[index].Socket.GetRemoteAddress(),
                                        (object) Netplay.Clients[index].Name);
                                }
                                catch (Exception ex)
                ...
                if (!WorldGen.saveLock && !Main.dedServ)
                    Main.statusText = num3 != 0
                        ? Language.GetTextValue("Net.ClientsConnected", (object) num3)
                        : Language.GetTextValue("Net.WaitingForClients");
