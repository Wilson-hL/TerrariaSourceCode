...
namespace Terraria.Social.Steam
{
    ...
    public class NetClientSocialModule : NetSocialModule
    {
        ...
        {
            base.Initialize();
            this._gameLobbyJoinRequested =
                Callback<GameLobbyJoinRequested_t>.Create(
                    new Callback<GameLobbyJoinRequested_t>.DispatchDelegate(OnLobbyJoinRequest));
            this._p2pSessionRequest =
                Callback<P2PSessionRequest_t>.Create(
                    new Callback<P2PSessionRequest_t>.DispatchDelegate(OnP2PSessionRequest));
            this._p2pSessionConnectfail =
                Callback<P2PSessionConnectFail_t>.Create(
                    new Callback<P2PSessionConnectFail_t>.DispatchDelegate(OnSessionConnectFail));
        ...
        {
            if (this._hasLocalHost || this._lobby.State == LobbyState.Active || Main.LobbyId != 0UL)
        ...
        private void OnLobbyJoinRequest(GameLobbyJoinRequested_t result)
        {
            ...
            Main.OpenPlayerSelect((Main.OnPlayerSelected) (playerData =>
            {
                ...
                this._lobby.Join((CSteamID) result.m_steamIDLobby,
                    new CallResult<LobbyEnter_t>.APIDispatchDelegate(OnLobbyEntered));
            }));
        }
